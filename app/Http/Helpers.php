...
if(!function_exists('get_coupon')){
 function get_coupon($id){
          $coupons=Coupon::where('end_date','>',strtotime(date('d-m-Y')))->where('type','!=','cart_base')->get();

          foreach($coupons as $coupon){
              $coupon_details = json_decode($coupon->details);
             if($coupon_details !=null){
              foreach( $coupon_details as $details){
                   if($coupon['count']>$coupon['count_use']){
                       if($details->product_id==$id){
               return $coupon['code'];
                       }
...
if (! function_exists('home_price')) {
    function home_price($id, $type = null)
    {
...
if (! function_exists('home_discounted_price')) {
    function home_discounted_price($id, $type = null)
    {
        $product = Product::findOrFail($id);
        if($type == 'wholesale_price') {
            $price = $product->wholesale_price;
...
if (! function_exists('home_base_price')) {
    function home_base_price($id,$type=null, $format=false)
    {
        $product = Product::findOrFail($id);
        $price = $product->unit_price;
        if($type == 'wholesale_price') {
...
if(!function_exists('price_wholesale')){
     function price_wholesale($id){
        $product = Product::findOrFail($id);

        $price = $product['wholesale_price'] - $product['wholesale_discount'];
        return format_price(convert_price($price));
...
if (! function_exists('home_discounted_base_price')) {
    function home_discounted_base_price($id,$type =null, $format = false)
    {
        $product = Product::findOrFail($id);
        $price = $product->unit_price;
        if($type == 'wholesale_price') {

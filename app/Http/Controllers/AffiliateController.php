...
class AffiliateController extends Controller
{
    ...
    public function store_affiliate_user(Request $request){
        if(!Auth::check()){
            ...
            $newAffiliate = Auth::user();
            $newAffiliate->customer->destroy($newAffiliate->customer->id);
            $newAffiliate->user_type = 'affiliate';
            $newAffiliate->save();
    ...
    {
        $affiliate_user = AffiliateUser::findOrFail($request->id);
        $orders_should_payments = orders_should_payments::where('user_id', $affiliate_user->user->id)
        ->where('paid', 0)
        ->where('refund', 0)
        ->get();
        return view('affiliate.payment_modal', compact('affiliate_user', 'orders_should_payments'));
    }

    public function payment_store(Request $request){
        $affiliate_user = AffiliateUser::findOrFail($request->affiliate_user_id);
        if($affiliate_user) {
            foreach ($request->orders_should_pay as $orders_should_pay) {
                $orders_should_payment = orders_should_payments::find($orders_should_pay);
                if($orders_should_payment) {
                    if(get_setting('refund_request_time') > 0) {
                        $carbon = new Carbon($orders_should_payment->order_detail->created_at);
                        $order_detail = OrderDetail::select(['*', DB::raw('DATEDIFF(now(), created_at) as refund_days')])
                        ->where('id', $orders_should_payment->order_detail->id)
                        ->where(DB::raw('DATEDIFF(now(), created_at)'), '>', get_setting('refund_request_time'))
                        ->first();
                        if($order_detail) {
                            $affiliate_user->user->balance -= $orders_should_payment->amount;
                            $orders_should_payment->paid = 1;
                            $orders_should_payment->save();
                            $affiliate_user->user->save();
                            $affiliate_payment = new AffiliatePayment;
                            $affiliate_payment->affiliate_user_id = $request->affiliate_user_id;
                            $affiliate_payment->amount = $orders_should_payment->amount;
                            $affiliate_payment->payment_method = $request->payment_method;
                            $affiliate_payment->save();
                        } else {
                            flash(translate('You Should Pay For') . ' '
                            . $orders_should_payment->order_detail->order->code . ' '
                            . translate('After') . ' '
            ...
            flash(translate('there is something error !'))->error();
        return back();
        }
    }
    ...
    public function user_index(){
        ...
        $orders_should_payments = orders_should_payments::
        where('user_id', $affiliate_user->user->id)
        ->where('paid', 0)
        ->where('refund', 0)
        ->get();

        return view('affiliate.frontend.index', compact('affiliate_payments','affiliate_withdraw_requests', 'orders_should_payments'));
    }
    ...
    public function affiliate_withdraw_modal(Request $request)
    {
        ...
        ->where('refund', 0)
        ->get();
        return view('affiliate.affiliate_withdraw_modal', compact('affiliate_withdraw_request','affiliate_user', 'orders_should_payments'));
    }

    public function withdraw_request_payment_store(Request $request){
        $affiliateUser = AffiliateUser::find($request->affiliate_user_id);
        if($affiliateUser) {
            if($request->orders_should_pay) {
                foreach ($request->orders_should_pay as $orders_should_pay) {
                    $orders_should_payment = orders_should_payments::find($orders_should_pay);
                    if($orders_should_payment) {
                        $affiliateUser->user->balance -= $orders_should_payment->amount;
                        $affiliateUser->user->save();
                        $orders_should_payment->paid = 1;
                        $orders_should_payment->save();
                        $affiliate_payment = new AffiliatePayment;
                        $affiliate_payment->affiliate_user_id = $request->affiliate_user_id;

...
class OTPVerificationController extends Controller
{
    ...
    public function send_order_code($order){
        if($order->phone != null){
            sendSMS($order->phone, env('APP_NAME'), 'You order has been placed and Order Code is : '.$order->code);
        }
    }
    ...
    public function send_delivery_status($order){
        if($order->phone != null){
            sendSMS($order->phone, env('APP_NAME'), 'Your delivery status has been updated to '.$order->orderDetails->first()->delivery_status.' for Order code : '.$order->code);
        }
    }
    ...
    public function send_payment_status($order){
        if($order->phone != null){
            sendSMS($order->phone, env('APP_NAME'), 'Your payment status has been updated to '.$order->payment_status.' for Order code : '.$order->code);
        }
    }
}

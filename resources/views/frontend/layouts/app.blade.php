...
<!-- Favicon -->
    ...
    @if(\App\Language::where('code', Session::get('locale', Config::get('app.locale')))->first()['rtl'] == 1)
        ...
        <link rel="stylesheet" href="{{ static_asset('assets/css/rtl.css') }}">
    @endif
    ...
    <style>
        ...
        :root {
            --primary: {{ get_setting('base_color', '#ff6702') }};
            --hov-primary: {{ get_setting('base_hov_color', '#77be44') }};
            --soft-primary: {{ hex2rgba(get_setting('base_color','#ff6702'),.15) }};
...
<div class="aiz-main-wrapper d-flex flex-column">
    ...
    @if(Auth::check())
        <div class="chat-ball">
            @php
                $conversation = \App\Conversation::where('sender_id', Auth::user()->id)->first();
            @endphp
            @if($conversation)
...
<script>
    ...
    function search() {
        ...
        if (search.length > 0) {
            ...
            $.post('{{ route('search.ajax') }}', {_token: AIZ.data.csrf, search: search}, function (data) {
                if (data == '0') {
    ...
    function showAddToCartModal(id) {
        ...
        $('#addToCart').modal();
        $('.c-preloader').show();
        $.post('{{ route('cart.showCartModal') }}', {_token: AIZ.data.csrf, id: id, unit_price: "{{ request('unit_price') }}"}, function (data) {
            // console.log(data);
            $('.c-preloader').hide();
    ...
    function getVariantPrice() {
        console.log($('#option-choice-form input[name=quantity]').val());

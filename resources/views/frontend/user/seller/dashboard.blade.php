                  ...
                  <div class="row gutters-10">
                    ...
                    <div class="col-md-3">
                        <div class="bg-grad-1 text-white rounded-lg mb-4 overflow-hidden">
                            <div class="px-3 pt-3">
                                <div class="fs-20 fw-700">
                                    @php
                                        $total_earnings = \App\orders_should_payments::where('seller_id', Auth::user()->seller->id)->where('paid', 1)->pluck('amount')->sum();
                                    @endphp
                                  {{ single_price($total_earnings) }}
                                </div>
                    ...
                    <div class="col-md-3">
                        <div class="bg-grad-2 text-white rounded-lg mb-4 overflow-hidden">
                            <div class="px-3 pt-3">
                                ...
                                @endphp
                                <div class="fs-20 fw-700">{{ single_price($total) }}</div>
                                <div class="opacity-50">{{ translate('Total Sales')}}</div>
                              </div>
                              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
                    ...
                    <div class="col-md-3">
                        <div class="bg-grad-3 text-white rounded-lg mb-4 overflow-hidden">
                          <div class="px-3 pt-3">
                              <div class="fs-20  fw-700">
                                  {{ count(\App\OrderDetail::where('seller_id', Auth::user()->id)
                              ...
                              <div class="card-body">
                                  <table class="table aiz-table mb-0">
                                      ...
                                      <tr>
                                          ...
                                          <td>{{ count(\App\OrderDetail::where('seller_id', Auth::user()->id)->where('delivery_status', 'معلق')->get()) }}</strong></td>
                                      </tr>
                                      <tr>
                                          <td>{{ translate('Cancelled orders')}}:</td>
                                          <td>{{ count(\App\OrderDetail::where('seller_id', Auth::user()->id)->where('delivery_status', 'مرتجع')->get()) }}</strong></td>
                                      </tr>
                                      <tr>
                                          <td>{{ translate('Successful orders')}}:</td>
                                          <td>{{ count(\App\OrderDetail::where('seller_id', Auth::user()->id)
                                            ->where('payment_status', 1)->get()) }}</strong></td>
                                      </tr>
                                  </table>
                              </div>
                          </div>

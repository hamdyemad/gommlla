                              ...
                              <table class="table aiz-table mb-0">
                                  <thead>
                                      <tr>
                                          ...
                                          <th width="30%">{{ translate('Message')}}</th>
                                      </tr>
                                  </thead>
                                  <tbody>
                                      @foreach ($seller_withdraw_requests as $key => $seller_withdraw_request)
                                          <tr>
                                              ...
                                              @if($seller_withdraw_request->order_detail)
                                              <td>{{ $seller_withdraw_request->order_detail->order->code }}</td>
                                              @endif
                    ...
                    <form class="" action="{{ route('withdraw_requests.store') }}" method="post">
                        ...
                        <div class="modal-body gry-bg px-3 pt-3">
                            <div class="form-group">
                                <label for="orders_should_pay">{{ translate('select ordrs that you want to send request') }}</label>
                                <select class="form-control aiz-selectpicker" multiple required name="orders_should_pay[]" id="orders_should_pay"  data-live-search="true">
                                    @foreach ($orders_should_payments as $orders_should_payment)
                                        @if($orders_should_payment->order_detail)

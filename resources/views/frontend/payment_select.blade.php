...
<section class="mb-4">
    <div class="container text-left">
        <div class="row">
            <div class="col-lg-8 order-2 order-md-1">
                <form action="{{ route('payment.checkout') }}" class="form-default" role="form" method="POST" id="checkout-form">
                    ...
                    <div class="card shadow-sm border-0 rounded">
                        <div class="card-header p-3 justify-content-center">
                        ...
                        <div class="card-body text-center">
                                ...
                                @if (Auth::check())
                                    @if (\App\Addon::where('unique_identifier', 'offline_payment')->first() != null && \App\Addon::where('unique_identifier', 'offline_payment')->first()->activated)
                                        @foreach(\App\ManualPaymentMethod::all() as $method)
                                            <div class="col-6 col-md-4">
                                                <label class="aiz-megabox d-block mb-3">
                                                    <input value="{{ $method->heading }}" type="radio" name="payment_option" onchange="toggleManualPaymentData({{ $method->id }})" checked>
                                                    <span class="d-block p-3 aiz-megabox-elem">
                            ...
                            <div class="shadow-sm border-0 rounded mb-3 bg-white p-3 d-none">
                            ...
                            @if (Auth::check() && \App\BusinessSetting::where('type', 'wallet_system')->first()->value == 1)
                                @if(Auth::user()->balance != 0)
                                    ...
                                    <div class="text-center py-4">
                                        ...
                                        @else
                                        <button  type="button" onclick="use_wallet(this)" class="btn btn-primary fw-600">{{ translate('Pay with wallet')}}</button>
                                        @endif
                                    </div>
                                @endif
                            @endif
                    ...
                    <div class="alert alert-danger d-none polices">{{ translate('You need to agree with our policies') }}</div>
            ...
            <div class="col-lg-4 order-1 order-md-2 mt-4 mt-lg-0">
            ...
            let loading = `
                <div class="load-wrapp">
                    <div class="load-6">
                        ...
                        <div class="l-1 letter">.</div>
                        <div class="l-1 letter">.</div>
                        <div class="l-1 letter">جارى انشاء الطلب برجاء الأنتظار</div>
                        </div>
                    </div>
                </div>
            `;
            if($('#agree_checkbox').is(":checked")){
                $(".polices").addClass('d-none');
                $(el).html(loading);
                $('#checkout-form').submit();
            }else{
                AIZ.plugins.notify('danger','{{ translate('You need to agree with our policies') }}');
                $(".polices").removeClass('d-none');

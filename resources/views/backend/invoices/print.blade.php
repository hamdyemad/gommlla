...
<body>
					...
					<td style="    width: 55%;text-align: right;">
						@if (Auth::user()->user_type == 'seller')
							@if(Auth::user()->shop->logo != null)
								<img loading="lazy"  src="{{ uploaded_asset(Auth::user()->shop->logo) }}" height="100" style="display:inline-block;">
							@else
								<img loading="lazy"  src="{{ static_asset('assets/img/logo.png') }}" height="40" style="display:inline-block;">
							@endif
						@else
							@if($logo != null)
								<img loading="lazy"  src="{{ uploaded_asset($logo) }}" height="100" style="display:inline-block;">
							@else
								<img loading="lazy"  src="{{ static_asset('assets/img/logo.png') }}" height="100" style="display:inline-block;">
							@endif
						@endif
						<lapel style="display: block; margin-top: 5px;font-size: 20px;">
						    +972 2-279-4343 <span>:الهاتف </span></lapel>
						    	<lapel style="display: block; margin-top: 5px;font-size: 20px;">
						    +972 54-464-4510<span>:الواتس  </span></lapel>
						  
						     <h2 style="margin-right: -87px;"> العيزرية الشارع الرئيسي بالقرب من البنك العربي</h2>
						    
					</td>
					<td style="font-size: 2.5rem;" class="text-right strong">{{ translate('INVOICE') }}
					<lapel style="    display: block" class="text-right small"><span class="gry-color small">{{  translate('Order ID') }}:</span> <span class="strong">{{ $order->code }}</span></lapel>
						<lapel style="    display: block" class="text-right small"><span class="gry-color small">{{  translate('Order Date') }}:</span> <span class=" strong">{{ date('d-m-Y', $order->date) }}</span></lapel>
					<br>
   ...
   <!-- 				</tr>-->
		...
		<!--    <h2> العيزرية الشارع الرئيسي بالقرب من البنك العربي</h2>    </div>-->

		<div style="padding:10px 55px;padding-bottom: 0">
            <table>
				@php
					$shipping_address = json_decode($order->shipping_address);
				@endphp
				<tr><td class="strong small gry-color">{{ translate('Bill to') }}:</td></tr>
				<tr><td class="strong">{{isset($shipping_address->name)? $shipping_address->name:'' }}</td></tr>
				<tr><td class="gry-color small">{{ isset($shipping_address->address)? $shipping_address->address:''}}, {{ isset($shipping_address->city)?$shipping_address->city:'' }}, {{ isset($shipping_address->country)?$shipping_address->country:'' }}</td></tr>
				<tr><td class="gry-color small">{{ translate('Email') }}: {{ isset($shipping_address->email)?$shipping_address->email:'' }}</td></tr>
				<tr><td class="gry-color small">{{ translate('Phone') }}: {{ isset($shipping_address->phone)?$shipping_address->phone:'' }}</td></tr>
			</table>
		</div>
	    ...
	    <div style="padding:0 1.5rem;">
	        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
		        <tbody>
			        <tr>
			            <th class="gry-color text-left">{{ translate('boint order') }}</th>
			            <td class="currency">{{$last_boint['points'] }}</td>
			        </tr>
			         <tr>
			            <th class="gry-color text-left">{{ translate('all boint') }}</th>
			            <td class="currency">{{$all_boint}}</td>
			        </tr>
		...
		<div style="text-align:center;padding:20px"><h2>    رجاء الاحتفاظ بالوصل لغاية تبديل خلال ثلاث ايام من تاريخة </h2>    </di
	</div>
</body>

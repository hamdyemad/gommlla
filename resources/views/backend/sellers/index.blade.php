                ...
                @if($seller->user != null)
                    <tr>
                        ...
                        <td>
                            ...
                            @else
                                {{ single_price($seller->admin_to_pay) }} ({{ translate('Due to Admin') }})
                            @endif
                        </td>
                        <td>
                          <div class="aiz-topbar-item ml-2">
                              <div class="align-items-stretch d-flex dropdown">
                                  ...
                                  <div class="dropdown-menu dropdown-menu-right dropdown-menu-animated dropdown-menu-xs">
                                      ...
                                      @php
                                          $conversation = \App\Conversation::where('receiver_id', $seller->user->id)->first();
                                      @endphp
                                      @if($conversation)
                                            <a href="{{ route('conversations.admin_show', encrypt($conversation->id)) }}"  class="dropdown-item">
                                                {{translate('Message')}}
                                            </a>
                                        @else
                                            <form action="{{ route('conversations.store') }}" method="POST">
                                                <input type="hidden" name="id" value="{{ $seller->user->id }}">
                                                @csrf
                                                <button class="dropdown-item">{{ translate('Message') }}</button>
                                            </form>
                                        @endif

...
<div class="card">
    <div class="card-header">
        ...
        <div class="form-group text-center">
            <label class="d-block" for="">{{ translate('Categories Show') }}</label>
            <label class="aiz-switch aiz-switch-success mb-0">
                <input type="checkbox" onchange="update_show(this)" value="{{ get_setting('categories_show') }}" <?php if(get_setting('categories_show') == 1) echo "checked";?>>
                <span></span>
            </label>
        </div>
        ...
        <table class="table table-responsive aiz-table mb-0">
            <thead>
                <tr>
                    ...
                    <th>{{translate('Showing Number')}}</th>
...
@section('script')
    <script type="text/javascript">
        ...
        function update_show(el){
            ...
            $.post('{{ route('categories_show') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Categories Header Updated') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
